...
local SWORD_API_KEYS = {
    GetSword = true, EquipSwordTo = true, GetInstance = true,
    SwordMounts = true, Swords = true, _swordListByRarity = true,
    GetSlashName = true, SetSword = true, GetSwordData = true,
    GetConfig = true, GetTool = true, CloneSword = true,
}

local function getSwordsNode()
    local shared = ReplicatedStorage:FindFirstChild("Shared")
    local ri = shared and shared:FindFirstChild("ReplicatedInstances")
    return ri and ri:FindFirstChild("Swords") or nil
end

local function isSwordName(swordsModule, swordsNode, name)
    if not name or name == "" or SWORD_API_KEYS[name] then return false end
    if name:sub(1, 1) == "_" then return false end
    if swordsNode and swordsNode:IsA("Instance") and swordsNode:FindFirstChild(name) then
        return true
    end
    if swordsModule and type(swordsModule.GetSword) == "function" then
        local ok, data = pcall(function() return swordsModule:GetSword(name) end)
        if ok and data ~= nil and data ~= false then return true end
        ok, data = pcall(function() return swordsModule.GetSword(name) end)
        if ok and data ~= nil and data ~= false then return true end
    end
    return false
end
...
local function collectSwordNames()
    local names = {}
    local seen = {}
    local function push(name, instance)
        if type(name) ~= "string" or name == "" or seen[name] then return end
        if SWORD_API_KEYS[name] or name:sub(1, 1) == "_" then return end
        seen[name] = true
        TableInsert(names, { Name = name, Instance = instance })
    end

    local swordsNode = getSwordsNode()
    local swordsModule = safeRequire("Shared.ReplicatedInstances.Swords")

    -- physical models under Swords (ModuleScript/Folder children)
    if swordsNode then
        for _, child in swordsNode:GetChildren() do
            if child:IsA("Model") or child:IsA("Folder") or child:IsA("Tool") or child:IsA("BasePart") or child:IsA("Configuration") then
                push(child.Name, child)
            end
        end
    end

    -- module data tables that map sword name -> config
    if type(swordsModule) == "table" then
        -- named lists (e.g. rarity maps, plain arrays of names)
        for listKey in {"_swordListByRarity", "SwordList", "AllSwords", "Swords"} do
            local list = swordsModule[listKey]
            if type(list) == "table" then
                for k, v in list do
                    if type(k) == "string" and k ~= "" and not SWORD_API_KEYS[k] and k:sub(1, 1) ~= "_" then
                        -- map: key is name if value is config; else value may be name
                        local valType = type(v)
                        if valType == "table" or valType == "string" or typeof(v) == "Instance" then
                            if valType == "string" and not isSwordName(swordsModule, swordsNode, k) then
                                push(v, nil)
                            else
                                push(k, typeof(v) == "Instance" and v or (swordsNode and swordsNode:FindFirstChild(k) or nil))
                            end
                        end
                    elseif type(v) == "string" and not SWORD_API_KEYS[v] and v:sub(1, 1) ~= "_" then
                        push(v, nil)
        ...
        -- flat module keys only if GetSword validates them (drops GetSword/EquipSwordTo/…)
        for key, value in swordsModule do
            if type(key) == "string" and not SWORD_API_KEYS[key] and key:sub(1, 1) ~= "_" then
                local valueType = type(value)
                local looksLikeData = valueType == "table" or typeof(value) == "Instance"
                if looksLikeData and isSwordName(swordsModule, swordsNode, key) then
                    push(key, typeof(value) == "Instance" and value or (swordsNode and swordsNode:FindFirstChild(key) or nil))
                end
            end
        end
    end

    -- last resort: shop-style scan already covered by children; keep unique
    return names
end

-- Weapons: swords (validated) + explosions + emotes + generic tools
local function fetchWeapons()
    ...
    local function add(name, instance, category)
        ...
        if SWORD_API_KEYS[name] then return end
        seen[name] = true
        TableInsert(weapons, {
            Name = name,
            Instance = instance,
    ...
    local okSwords, swordList = pcall(collectSwordNames)
    if okSwords and type(swordList) == "table" then
        for _, entry in swordList do
            add(entry.Name, entry.Instance, "sword")
        end
    end
...
local function fetchCosmetics(weapon)
    ...
    if category == "sword" then
        if #cosmetics == 0 then
            local okList, swordList = pcall(collectSwordNames)
            if okList and type(swordList) == "table" then
                for _, s in swordList do
                    if not obj or (not s.Instance) or s.Instance ~= obj then
                        add(s.Name, s.Instance)
                    end
                end
            end
        end
...
local function buildGroupCard(parent, label, count, layoutOrder, isExpanded, onToggle)
    local card = create("TextButton", {
        Parent = parent,
        Name = "\0",
        FontFace = Font,
    ...
    corner(card, 5)
    local cardStroke = stroke(card, isExpanded and Theme.Accent or Theme.Outline, isExpanded and 1.5 or 1)
    ...
    create("TextLabel", {
        ...
        Name = "\0",
        FontFace = Font,
        Text = (isExpanded and "▾ " or "▸ ") .. label,
        TextColor3 = isExpanded and Theme.Text or Theme["Dark Text"],
        TextSize = 10,
        TextWrapped = true,
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
...
local function firstWordKey(name)
    local lower = StringLower(name)
    local word = lower:match("^([%a]+)")
    return word or lower
end

local function groupDisplayList(weapons)
    local filter = StringLower(State.WeaponFilter)
    local filtered = {}
    for _, w in weapons do
        local catOk = State.Category == "all" or w.Category == State.Category
        local textOk = filter == "" or StringLower(w.Name):find(filter, 1, true)
        if catOk and textOk then
            TableInsert(filtered, w)
        end
    end

    local counts = {}
    for _, w in filtered do
        local k = w.Category .. "\0" .. firstWordKey(w.Name)
        counts[k] = (counts[k] or 0) + 1
    end

    local groupsByKey = {}
    local groupOrderKeys = {}
    local singles = {}

    for _, w in filtered do
        local word = firstWordKey(w.Name)
        local k = w.Category .. "\0" .. word
        if (counts[k] or 0) >= 2 then
            local g = groupsByKey[k]
            if not g then
                g = { Kind = "group", Key = k, Label = word, Category = w.Category, Members = {} }
                groupsByKey[k] = g
                TableInsert(groupOrderKeys, g)
            end
            TableInsert(g.Members, w)
        else
            TableInsert(singles, w)
        end
    end

    table.sort(groupOrderKeys, function(a, b) return a.Label < b.Label end)
    table.sort(singles, function(a, b) return a.Name:lower() < b.Name:lower() end)
    for _, g in groupOrderKeys do
        table.sort(g.Members, function(a, b) return a.Name:lower() < b.Name:lower() end)
    end

    local display = {}
    local gi, si = 1, 1
    while gi <= #groupOrderKeys or si <= #singles do
        local takeGroup = false
        if gi <= #groupOrderKeys and si <= #singles then
            takeGroup = groupOrderKeys[gi].Label <= singles[si].Name:lower()
        elseif gi <= #groupOrderKeys then
            takeGroup = true
        end
        if takeGroup then
            TableInsert(display, groupOrderKeys[gi])
            gi += 1
        else
            TableInsert(display, singles[si])
            si += 1
        end
    end
    return display
...
local function toggleWeaponEnabled(weapon)
...
refreshWeaponGrid = function()
    ...
    local display = groupDisplayList(State.Weapons)
    local filterActive = StringLower(State.WeaponFilter) ~= ""
    local slot = 0
    ...
    for _, entry in display do
        if entry.Kind == "group" then
            local expanded = State.ExpandedGroups[entry.Key] == true or filterActive
            local baseOrder = nextSlot()
            local groupCard = buildGroupCard(
                weaponSection.GridHolder, entry.Label, #entry.Members, baseOrder, expanded,
                function()
                    State.ExpandedGroups[entry.Key] = not (State.ExpandedGroups[entry.Key] == true)
                    refreshWeaponGrid()
                end
            )
            TableInsert(weaponSection.Cards, groupCard)

            if expanded then
                for i, weapon in entry.Members do
                    local isSelected = State.SelectedWeapon and State.SelectedWeapon.Name == weapon.Name
                    local card = buildCard(
                        weaponSection.GridHolder, weapon, baseOrder + i, isSelected,
                        function() selectWeapon(weapon) end,
                        function() toggleWeaponEnabled(weapon) end
                    )
            ...
            local isSelected = State.SelectedWeapon and State.SelectedWeapon.Name == weapon.Name
            local card = buildCard(
                weaponSection.GridHolder, weapon, nextSlot(), isSelected,
                function() selectWeapon(weapon) end,
                function() toggleWeaponEnabled(weapon) end
            )
            TableInsert(weaponSection.Cards, card)
